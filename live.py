...
@catub.cat_cmd(
    ...
    info={
        "header": "To check bot's alive status, Plugin by [毒液先生](https://t.me/V_6_Y),[𝓉𝒽𝖊 𝓅𝓊𝓃𝒾𝓈𝒽𝖊𝓇](https://t.me/X_4_Y)",
...
async def amireallyalive(event):
    ...
    if CAT_IMG:
        ...
        cat_caption = f"**{random.choice(ANIME_QUOTE)}**\n\n"
        cat_caption += f"۝⩵꙰ཱི►**`𝙱𝙾𝚃-𝙲𝙰𝚃` 🍓😻**◄⩵꙰ཱི۝\n Base on 🤖 `{check_sgnirts}` 🤖\n"
        cat_caption += f"┏━━━━━━━✦❘༻༺❘✦━━━━━━━┓\n"
        cat_caption += f"┃‣ 👤 **Master :** {mention}🔥\n"
        "`┣▰▱▰▱▰▱▰▱▰▱▰▱▰▱▰▱`\n"
        cat_caption += f"┃‣ ⚙️ **Telethon :** `{version.__version__}`🔥\n"
        cat_caption += f"┃‣ 🐍 **Python :** `{python_version()}`🔥\n"
        cat_caption += f"┃‣ 💻 **Database :** `{check_sgnirts}`🔥\n"
        cat_caption += f"┃‣ 🐈 **Cat Version :** `{catversion}`🔥\n"
        cat_caption += f"┃‣ 🕒 **Uptime :** `{uptime}`🔥\n"
        cat_caption += f"┗━━━━━━━✦❘༻༺❘✦━━━━━━━┛\n"
        cat_caption += f"┏━━━━━✦❘༻༺❘✦━━━━━┓\n┃‣ **📡 ᴘɪɴɢ :** {ms} ms \n┗━━━━━✦❘༻༺❘✦━━━━━┛\n"
        cat_caption += f" ‣ **💙 ᴄʀᴇᴅɪᴛs ᴛᴏ ᴛʜᴇ ᴄʀᴇᴀᴛᴏʀs ᴏғ ᴄᴀᴛᴜꜱᴇʀʙᴏᴛ & sᴜᴘᴘᴏʀᴛᴇʀs"
